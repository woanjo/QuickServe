...
if (isset($_POST['action_signup']) && isset($_POST['signup_id'])) {
    ...
    // Verify this is admin's mission and get volunteer details
    $checkStmt = $pdo->prepare("
        SELECT s.*, m.title as mission_title, m.admin_id, m.mission_date, m.hours, 
    ...
    if ($signupData && $signupData['admin_id'] == $adminId) {
        ...
        $email = EmailConfig::getInstance();
        $emailSent = $email->sendEmail(
            $signupData['volunteer_email'],
            $signupData['volunteer_name'],
            $signupData['mission_title'],
            $newStatus,
            date('F j, Y', strtotime($signupData['mission_date'])),
            $signupData['hours']
        );

        if($emailSent) {
            $success = "Signup " . ($newStatus === 'approved' ? 'approved' : 'rejected') . " successfully! Email sent to volunteer.";
        } else {
            $success = "Signup " . ($newStatus === 'approved' ? 'approved' : 'rejected') . " successfully! However, there was an error sending the email notification.";
        }
    }
}
...
if (isset($_POST['confirm_completion']) && isset($_POST['signup_id'])) {
    ...
    if ($signupData && $signupData['admin_id'] == $adminId) {
        // COMPLETED TIMESTAMP
        $stmt = $pdo->prepare("UPDATE signups SET completed = 1, completed_at = NOW() WHERE id = ?");
        ...
        if ($emailSent) {
            $success = "Volunteer hours confirmed! Email sent to volunteer.";
        } else {
            $success = "Volunteer hours confirmed but email failed to send";
        }
    }
}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" style="display: inline;">
                                                ...
                                                <button type="submit" name="action_signup" value="approve" class="btn btn-small btn-success">✅ Approve & Send Email</button>
                                                <button type="submit" name="action_signup" value="reject" class="btn btn-small btn-danger">❌ Reject & Send Email</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                ...
                                <?php foreach ($completionPending as $signup): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" style="display: inline;">
                                                <input type="hidden" name="signup_id" value="<?php echo $signup['id']; ?>">
                                                <button type="submit" name="confirm_completion" class="btn btn-small btn-success">✅ Confirm Hours & Send Email</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
